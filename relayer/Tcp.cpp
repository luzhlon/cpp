...
int TcpConnect::Send(const char *buf, int len, int flags)
{
	const char *p = buf;
	do {
		int n = ::send(m_socket, p, len, flags);
		if (n < 0) return n;
		len -= n;
		p += n;
	} while (len);
...
int TcpConnect::Recv(char *buf, int len, int flags)
{
	char *p = buf;
	do {
		int n = ::recv(m_socket, p, len, flags);
		if (n < 0) return n;
		len -= n;
		p += n;
	} while (len);
